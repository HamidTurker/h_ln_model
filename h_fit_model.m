...
for k = 1:numFolds
    ...
    if k == 1
        init_params = 1e-3*randn(numCol, 1);
    else
        init_params = params;
    end
    [params] = fminunc(@(params) h_ln_poisson_model(params,data,modelType,ln_params,ln_params_dims),init_params,opts);
    
    %%%%%%%%%%%%% TEST DATA %%%%%%%%%%%%%%%%%%%%%%%
    % compute the firing rate
    fr_hat_test = exp(test_A * params)/dt;
    ...
    r = exp(test_A * params); n = test_spikes; meanFR_test = nanmean(test_spikes); 
    ...
    %%%%%%%%%%%%% TRAINING DATA %%%%%%%%%%%%%%%%%%%%%%%
    % compute the firing rate
    fr_hat_train = exp(train_A * params)/dt;
    ...
    r_train = exp(train_A * params); n_train = train_spikes; meanFR_train = nanmean(train_spikes);   
